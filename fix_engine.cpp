    ...
    while (true) {
        ...
        if (clientSocket < 0) {
            ...
            try {
                ...
                session->acceptor = this;
                session->thread = new std::thread(&Session::handle, session);
...
void Session::handle() {
    ...
    try {
        while (true) {
            ...
            if (msg.getString(tagValue(Tags::SENDER_COMP_ID)) !=
                config.targetCompId) {
...
void Initiator::handle() {
    ...
    while (!is.eof()) {
        FixMessage::parse(is, msg, GroupDefs());
        // std::cout << "client received message! type "<<msg.msgType()<<", eof
        // "<<is.eof()<<"\n";
